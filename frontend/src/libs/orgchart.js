...
export default class OrgChart {
    ...
    _createNode(nodeData, level) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            nodeDiv.innerHTML = `
        <div class="title" id="${nodeData.id}"></div>
        ${opts.nodeContent ? `<div class="content"></div>` : ''}
      `;
            // append 4 direction arrows or expand/collapse buttons
            let flags = nodeData.relationship || '';

            //#ef:changed button layout
            ...
            if (opts.verticalDepth && (level + 2) > opts.verticalDepth) {
                if ((level + 1) >= opts.verticalDepth && Number(flags.substr(2, 1))) {
                    ...
                    toggleBtn.addAttribute('class', 'toggleBtn fa fa-' + icon + '-square');
                ...
                if (Number(flags.substr(2, 1))) {
                    ...
                    bottomEdge.setAttribute('id',nodeData.id)
                    bottomEdge.setAttribute('class', 'edge verticalEdge bottomEdge fa');
